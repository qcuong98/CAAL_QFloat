...
Number Number::cos() const {
    Number x(*this - (*this / (two * PI)).floor() * PI * two);
    x = x * x;
    Number T(one);
    Number M(one);
    Number tmp(one);
...
Number Number::asin() const {
    if (*this > one || *this < -one)
        throw Exception("NaN");
    if (one - abs() < Number("0.05")) {
        Number x(abs());
        x.d[0] = 0;
        x      = ((one - x * x) ^ Number("0.5")) / (x ^ (one / Number("3")));
        if (sign < 0)
            x = PI - x;
        return PI / two - x;
    }
